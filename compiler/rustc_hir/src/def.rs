...
pub enum DefKind {
    ...
    TyAlias,
    ForeignTy,
    /// Trait alias: `type IntIterator = Iterator<Item = i32>;`
    TraitAlias,
    /// Associated type: `trait MyTrait { type Assoc; }`
    AssocTy,
    /// Type parameter: the `T` in `Vec<T>`
    TyParam,
    ...
    Use,
    ForeignMod,
    /// Anonymous `const`, e.g. `const { 1 + 2}`
    AnonConst,
    /// Opaque type, aka `impl Trait`.
    OpaqueTy,
    Field,
    /// Lifetime parameter: the `'a` in `&'a str`
    LifetimeParam,
    /// A use of [`global_asm!`].
    GlobalAsm,
    Impl,
...
pub enum Res<Id = hir::HirId> {
    /// Any ordinary definition not covered by the other `Res` variants.
    ///
...
pub enum Namespace {
    ...
    TypeNS,
    /// The value namespace includes `fn`s, `const`s, and `static`s.
    ValueNS,
